...
io.on('connection', (socket) => {
    ...
    socket.on('update-note', (data) => {
        try {
            ...
            if (room && room.players[socket.id]) { // Prüft ob der Sender ein Spieler ist
    ...
    socket.on('update-points', (data) => {
        try {
            ...
            if (room && room.host === socket.id) {  // Prüft ob der Sender der Host ist
    ...
    socket.on('disconnect', () => {
        try {
            if (currentRoom && rooms[currentRoom]) {
                ...
                const connectedClients = io.sockets.adapter.rooms.get(currentRoom)?.size || 0;
                if (connectedClients === 0) {
                    deleteRoom(currentRoom);
                }
            }
